...
import { operatorTypeMap, type OperatorTypeKey } from "./admin.constants.js";
...
type UpdateUserInput = Partial<UserInput> & { id: string };

function assertOperatorNumber(operatorType: OperatorTypeKey, operatorNumber: string) {
  const allowed = operatorTypeMap[operatorType].values;
  if (!allowed.includes(operatorNumber as never)) {
...
export async function listUsers() {
  const result = await pool.query(
    `SELECT id, username, operator_type_key AS "operatorType", operator_type_label AS "operatorTypeLabel",
...
export async function createUser(input: UserInput) {
  ...
  const result = await pool.query(
    ...
    RETURNING id, username, operator_type_key AS "operatorType",
      ...
      operator_number AS "operatorNumber", created_at AS "createdAt"`,
    [input.username, passwordHash, input.operatorType, label, input.operatorNumber]
  );
...
export async function updateUser(input: UpdateUserInput) {
  const currentResult = await pool.query(
    `SELECT operator_type_key AS "operatorType" FROM admin_users WHERE id = $1`,
    [input.id]
  );
  ...
  const operatorType = (input.operatorType ??
    currentResult.rows[0].operatorType) as OperatorTypeKey;
...
export async function listModelNumbers() {
  const result = await pool.query(
    `SELECT id, model_number AS "modelNumber", created_at AS "createdAt"
...
export async function createModelNumber(modelNumber: string) {
  const result = await pool.query(
    `INSERT INTO admin_model_numbers (model_number)
     VALUES ($1)
     RETURNING id, model_number AS "modelNumber", created_at AS "createdAt"`,
    [modelNumber]
  );
  return result.rows[0];
}

export async function deleteModelNumber(id: string) {
